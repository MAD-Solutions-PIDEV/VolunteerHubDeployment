...
import { addFavoriteHost, getListHost, getListUser, getUserById } from "BackEnd/Modules/services/userService";
...
const SingleHost = ({ project = {},user = {} }) => {
  ...
  const {tagline ,firstName,lastName,country,availableServices, languageSpoken, _id, address,image} = user;
  ...
  const handleClick  = async (e) => {
    try {
      if (isClicked) {
        ...
        const res = await axios.post(`http://localhost:4000/user/removefavoritehost/${storedId}`, Host);
        console.log(res.data);
      } else {
        // Call the API to add the host to the user's favorite list
        const res = await axios.post(`http://localhost:4000/user/favoriteHost/${storedId}`, Host);
        console.log(res.data);
      }
  ...
  return (
    <div className="explore-projects-item mt-50 ml-70 mr-160 " style={{ border: "1px dotted " }}  >
       
      <div className="explore-projects-thumb "   >
        
      <div className="explore-projects-content "  style={{ border: "1px dotted " }}  >
...
{/*         
   ...
   */}
    ...
    >
      ...
      {isClicked ? 'Remove from favorite host list' : 'Add to favorite host list'}
