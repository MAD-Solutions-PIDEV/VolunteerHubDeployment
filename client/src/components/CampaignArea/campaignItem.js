    ...
    <Col lg={campaignTwo ? 4 : 3} md={campaignTwo ? 7 : 6}>
      <div
        className={`campaign-item mt-30${
          !campaignTwo && isEven ? " d-flex flex-column flex-md-column-reverse" : ""
        }`}
      >
        <div className="campaign-thumb">
        <Image src={`http://localhost:4000/uploads/${image}`} fluid/>
        </div>
        <div className="campaign-content">
          <span>Deadline: { deadline ? format(new Date(deadline), 'YYYY-MM-DD') : '' }</span>
          <ul>
            <li>
          ...
          <button onClick={() => handleViewCampaign(campaign._id)}><i className="flaticon-next" ></i> View Details</button>
